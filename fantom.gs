...
hack = function(ip,port)
	...
	exhandler = function(addr,unsec)
		...
		if typeof(ex) == "shell" then
			print("Fantom has found a shell vuln\n what do you want to do?")
			print("1. Grab user banks")
			print("2. Start terminal")
			said = user_input("Answer:")
			...
			if said == "1" then
				...
				system_message("You should see banks here if it was a success.")
				system_message("For some reason sometimes the password doesnt appear")
				print("In order to combat this the raw content is shown too")
				...
				for user in home.get_folders
					...
					if bankFile != null then
						...
						if bankFile.has_permission("r") then
							...
							info = bankFile.content.split(":")
							password = crypto.decipher("root",info[1])
							numbers = info[0]
							
							print(numbers+" > "+password)
							exhandler(addr,unsec)
					end if
						system_message("Failure they're no bank files.")
						exhandler(addr,unsec)
					end if
					exhandler(addr,unsec)
				end if
